/**
 * Environment for Playwright critical-path e2e.
 *
 * Prefer a pre-seeded capability token (`E2E_SHARE_TOKEN`) for share-only smoke.
 * Full critical path (trip / enrich / upload / export / share) needs
...
export interface E2EEnv {
   ...
   * When set, share smoke opens `/s#token` without owner auth.
   */
  ...
  /**
   * Owner access token (Bearer). Required for critical path (create trip,
   * enrich, upload, export, share seed). Prefer a non-DPoP-bound token.
   */
  readonly ownerAccessToken: string | undefined;
  /**
   * When true, require successful S3 PUT + confirm for attachment step.
   * Default false: presign always asserted; PUT/confirm soft-skips on failure.
   * Env: `E2E_REQUIRE_ATTACHMENT_UPLOAD=true`.
   */
  readonly requireAttachmentUpload: boolean;
}
...
export function loadE2EEnv(
  ...
  return {
    ...
    requireAttachmentUpload: parseBoolFlag(
      env.E2E_REQUIRE_ATTACHMENT_UPLOAD,
      false,
    ),
  };
}
...
/**
 * True when the suite has enough credentials for share smoke.
...
/**
 * Full critical path needs owner Bearer (create / enrich / upload / export).
 */
export function hasCriticalPathCredentials(env: E2EEnv): boolean {
  return env.ownerAccessToken !== undefined;
}
...
export function criticalPathSkipReason(env: E2EEnv): string | undefined {
  if (hasCriticalPathCredentials(env)) {
    return undefined;
  }
  return (
    "Critical path skipped: set E2E_OWNER_ACCESS_TOKEN (non-DPoP Bearer). " +
    "See packages/e2e/README.md."
  );
}

/** Any e2e suite can run (share smoke and/or critical path). */
export function hasAnyE2ECredentials(env: E2EEnv): boolean {
  return hasShareSmokeCredentials(env) || hasCriticalPathCredentials(env);
}
