...
describe("itinerary item CRUD + reorder", () => {
  ...
  it("PATCH startAt/endAt null clears times (unscheduled)", async () => {
    const { tripRepo, tripId } = await createTrip();
    const created = await createItem(tripRepo, tripId, flightBody("NH100"));
    const item = JSON.parse(created.body ?? "{}") as {
      itemId: string;
      version: number;
      startAt?: string;
    ...
    const cleared = await Effect.runPromise(
      handleRequest(
        baseRequest({
          ...
          path: `/api/v1/trips/${tripId}/items/${item.itemId}`,
          headers: { "if-match": `"${item.version}"` },
          body: JSON.stringify({ startAt: null, endAt: null }),
        }),
        deps(tripRepo),
      ),
    );
    expect(cleared.status).toBe(200);
    const body = JSON.parse(cleared.body ?? "{}") as {
      startAt?: string;
      endAt?: string;
      version: number;
    };
