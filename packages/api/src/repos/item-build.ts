...
/**
 * Normalized patch fields. Instant fields use a 3-way:
 * - omitted (`undefined`) — leave existing
 * - `string` — set
 * - `null` — clear (REMOVE attribute)
 */
export type ResolvedItemPatchFields = {
 ...
 * `startAt` / `endAt`: `null` clears; string sets; omitted leaves unchanged.
 */
export function resolveItemPatchFields(
  existing: ItineraryItem,
  patch: UpdateItineraryItem,
): ResolvedItemPatchFields {
  const out: {
    title?: string;
    startAt?: string | null;
    endAt?: string | null;
 ...
 * Cleared Instant fields use `REMOVE` (JSON `null` on the patch).
 */
export function buildItemPatchUpdateExpression(
  ...
  if (fields.startAt === null) {
    removes.push("startAt");
  } else if (fields.startAt !== undefined) {
    sets.push("startAt = :startAt");
    values[":startAt"] = fields.startAt;
  }
  if (fields.endAt === null) {
    removes.push("endAt");
  } else if (fields.endAt !== undefined) {
